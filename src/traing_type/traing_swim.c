...
static void swim_screen_cntx_init()
{
    if ((swim_screen_cntx.height == 0) && (swim_screen_cntx.width==0)) {

        bsp_lcd_get_parameter(LCM_IOCTRL_QUERY__LCM_HEIGHT, &(swim_screen_cntx.height));
        bsp_lcd_get_parameter(LCM_IOCTRL_QUERY__LCM_WIDTH, &(swim_screen_cntx.width));
//        LOG_I("width(%d), height(%d).\r\n", fota_screen_cntx.width, fota_screen_cntx.height);

        swim_screen_cntx.bg_color = 0;
        swim_screen_cntx.font_color = 0xFFFF;
        /* first */
        swim_screen_cntx.fota_title_x = 5;
        swim_screen_cntx.fota_title_y = 6;

        /* middle */
        swim_screen_cntx.hr_title_x = LEFT_GAP;
        swim_screen_cntx.hr_title_y = ITEM_HEIGHT*2 + swim_screen_cntx.fota_title_y;
        swim_screen_cntx.hrv_title_x = LEFT_GAP;
        swim_screen_cntx.hrv_title_y = ITEM_HEIGHT + swim_screen_cntx.hr_title_y;
        swim_screen_cntx.hrv_value_x = LEFT_GAP;
        swim_screen_cntx.hrv_value_y = ITEM_HEIGHT + swim_screen_cntx.hrv_title_y;

        /* end */
        swim_screen_cntx.bt_name_x = LEFT_GAP;
        swim_screen_cntx.bt_name_y = swim_screen_cntx.height - 6 - (ITEM_HEIGHT*3);
        swim_screen_cntx.bt_status_x = LEFT_GAP;
        swim_screen_cntx.bt_status_y = ITEM_HEIGHT*2 + swim_screen_cntx.bt_name_y;
        swim_screen_cntx.back_x1 = swim_screen_cntx.width-80;
        swim_screen_cntx.back_y1 = ITEM_HEIGHT*2 + swim_screen_cntx.bt_name_y;
        swim_screen_cntx.back_x2 = swim_screen_cntx.width-1;
        swim_screen_cntx.back_y2 = swim_screen_cntx.height-1;

		swim_screen_cntx.focus_point_index = 0;
		swim_screen_cntx.start_item = 0;
		swim_screen_cntx.one_screen_item_num = 2;
		swim_screen_cntx.total_item_num = 2;
		swim_screen_cntx.curr_item_num = 2;
    }

}

static uint8_t* swim_convert_string_to_wstring(char* string)
{
...
*/
		...
		GRAPHICLOG("[chenchen swim_screen_keypad_event_handler key state=%d, position=%d\r\n", (int)keypad_event->state, (int)keypad_event->key_data);
