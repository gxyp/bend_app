...
{
    if ((running_screen_cntx.height == 0) && (running_screen_cntx.width==0)) {

        bsp_lcd_get_parameter(LCM_IOCTRL_QUERY__LCM_HEIGHT, &(running_screen_cntx.height));
        bsp_lcd_get_parameter(LCM_IOCTRL_QUERY__LCM_WIDTH, &(running_screen_cntx.width));
//        LOG_I("width(%d), height(%d).\r\n", fota_screen_cntx.width, fota_screen_cntx.height);

        running_screen_cntx.bg_color = 0;
        running_screen_cntx.font_color = 0xFFFF;
        /* first */
        running_screen_cntx.fota_title_x = 5;
        running_screen_cntx.fota_title_y = 6;

        /* middle */
        running_screen_cntx.hr_title_x = LEFT_GAP;
        running_screen_cntx.hr_title_y = ITEM_HEIGHT*2 + running_screen_cntx.fota_title_y;
        running_screen_cntx.hrv_title_x = LEFT_GAP;
        running_screen_cntx.hrv_title_y = ITEM_HEIGHT + running_screen_cntx.hr_title_y;
        running_screen_cntx.hrv_value_x = LEFT_GAP;
        running_screen_cntx.hrv_value_y = ITEM_HEIGHT + running_screen_cntx.hrv_title_y;

        /* end */
        running_screen_cntx.bt_name_x = LEFT_GAP;
        running_screen_cntx.bt_name_y = running_screen_cntx.height - 6 - (ITEM_HEIGHT*3);
        running_screen_cntx.bt_status_x = LEFT_GAP;
        running_screen_cntx.bt_status_y = ITEM_HEIGHT*2 + running_screen_cntx.bt_name_y;
        running_screen_cntx.back_x1 = running_screen_cntx.width-80;
        running_screen_cntx.back_y1 = ITEM_HEIGHT*2 + running_screen_cntx.bt_name_y;
        running_screen_cntx.back_x2 = running_screen_cntx.width-1;
        running_screen_cntx.back_y2 = running_screen_cntx.height-1;

		running_screen_cntx.focus_point_index = 0;
		running_screen_cntx.start_item = 0;
		running_screen_cntx.one_screen_item_num = 2;
		running_screen_cntx.total_item_num = 2;
		running_screen_cntx.curr_item_num = 2;
    }

}

static void running_screen_keypad_event_handler(hal_keypad_event_t* keypad_event,void* user_data)
{
		int32_t temp_index;
...
static uint8_t* running_convert_string_to_wstring(char* string)
{
...
//   int32_t num = sensor_screen_cntx.curr_item_num;
	...
	running_screen_cntx_init();
	demo_ui_register_keypad_event_callback(running_screen_keypad_event_handler, false, NULL);
